...
build {
  ...
  #
  # Fetching from /releases/latest/download/ picks up the newest release
  # automatically — the raw-tarball asset's filename is version-free.
  # Upstream's own unit file assumes a different userland (User=admin),
  # so we ship our own from files/tart-guest-agent.service.
  provisioner "file" {
